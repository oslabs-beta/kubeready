...
const installControler = {
  ...
  installPrometheus: (req, res, next) => {
    ...
    // Adding the Prometheus repository to Helm
    ...
    // Updating the prometheus-community repository 
    ...
    // Installing the kube-prometheus-stack Helm chart from the prometheus-community repository
  ...
  recreatePromGraf: (req, res, next) => {
    ...
    // Getting a list of pods to find the full prometheus-grafana pod name, since this name varies by user
    ...
    // In an array - separating each line of the result of running "kubectl run pods"
    const kubectlGetPodsText = kubectlGetPods.stdout.split('\n');
    
    // Finding the pod that contains "prometheus-grafana"
    let pod;
    ...
    //  Deleting old prometheus-grafana pod
  ...
  portForward: (req, res, next) => {
    ...
    // Declaring a port to independently display and access Grafana
    const PORT = 3000;

    // Asyncronously forwarding prometheus-grafana to the PORT
    const portFw = spawn(
